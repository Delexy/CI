...
1. Настройте линтер для соответствия сообщений о коммитах формату conventional commits (Прошу понять и простить, я 4 часа потратил чтобы поднять это дело, файлы вы можете видеть в .husky/commit-msg и commitlint.config.ts, но оно не завелось)
